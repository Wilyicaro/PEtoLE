...
{
	m_entityData.define<int8_t>(DATA_SWELL_DIR, -1);
	m_entityData.define<int8_t>(DATA_POWERED, 0);
}
...
void Creeper::die(Entity* pCulprit)
{
	...
	if (pCulprit && pCulprit->getType() == EntityType::skeleton)
	{
...
void Creeper::checkHurtTarget(Entity* pEnt, float f)
{
	...
	if (swellDir <= 0 && f < 3.0f || swellDir > 0 && f < 7.0f)
	{
		...
		{
			m_pLevel->explode(this, m_pos, isPowered() ? 6.0f : 3.0f);
			remove();
		}
...
void Creeper::thunderHit(LightningBolt* bolt)
{
	Entity::thunderHit(bolt);
...
{
	return m_entityData.get<int8_t>(DATA_POWERED);
}
